...
- [x] タスク 3.9: サインアップ機能の実装

  - [x] 3.9.1 サインアップページの作成

    - `front/src/app/signup/page.tsx` を作成
    - メールアドレス、パスワード、名前、言語選択のフォームを実装
    - パスワード確認フィールドを追加
    - フォームバリデーション（メール形式、パスワード強度）を実装
    - _要件: 1.1, 1.2_

  - [x] 3.9.2 サインアップ API 連携の実装

    - `front/src/lib/api.ts` の `authApi` に `register` メソッドを追加
    - サインアップ成功時の自動ログイン処理を実装
    - エラーハンドリング（重複メール、パスワード不一致等）を実装
    - _要件: 1.1, 1.2, 4.1, 4.3_

  - [x] 3.9.3 AuthContext にサインアップ機能を追加

    - `front/src/contexts/AuthContext.tsx` に `register` 関数を追加
    - サインアップ後の自動認証状態更新を実装
    - ローディング状態とエラー状態の管理を追加
    - _要件: 1.1, 1.2_

  - [x] 3.9.4 サインアップフォームの UI 実装

    - レスポンシブデザインでフォームレイアウトを作成
    - パスワード強度インジケーターを実装
    - リアルタイムバリデーションメッセージを表示
    - 送信中のローディング状態を表示
    - _要件: 1.4, 3.1, 3.2_

  - [x] 3.9.5 ログイン画面からサインアップ画面への導線追加

    - `front/src/components/LandingHeader.tsx` のログインモーダルに「アカウント作成」リンクを追加
    - サインアップページからログインページへの「既にアカウントをお持ちの方」リンクを追加
    - _要件: 3.1, 3.2_

  - [x] 3.9.6 パスワード要件の実装

    - 最小 8 文字、小文字 1 つ以上、大文字 1 つ以上、数字 1 つ以上、特殊文字 1 つ以上の要件を設定
    - Cognito デフォルトパスワードポリシーに準拠（8 文字以上、大文字・小文字・数字・特殊文字すべて必須）
    - パスワード確認フィールドでの一致チェック
    - 要件を満たさない場合の詳細なエラーメッセージ表示
    - _要件: 1.2, 4.2_
  ...
  - [x] 3.9.7 言語選択機能の実装
    ...
    - 日本語・英語の選択ラジオボタンを追加
    - 選択した言語での UI 表示切り替え
    - サインアップ時に選択言語をユーザープロファイルに保存
    - _要件: 1.1, 1.4_

  - [x] 3.9.8 サインアップ成功時の処理実装

    - サインアップ成功後の自動ログイン
    - 初回ログイン時のウェルカムメッセージ表示
    - バウアー一覧画面への自動リダイレクト
    - _要件: 1.1, 3.3_

  - [x] 3.9.9 エラーハンドリングの実装
  ...
  - [x] 3.9.10 バックエンドのサインアップ機能確認

    - `back/internal/service/auth_service.go` の `Register` メソッドが正しく実装されているか確認
    - パスワードハッシュ化が適切に行われているか確認
    - ユーザー作成時のバリデーションが適切か確認
    - _要件: 1.1, 1.2, 4.2_

  - [x] 3.9.11 サインアップフローのテスト実装
...
### タスク 3.10: Cognito パスワードポリシー設定

- [ ] タスク 3.10: Cognito パスワードポリシー設定

  - [ ] 3.10.1 Cognito User Pool のパスワードポリシー確認

    - AWS Cognito コンソールでパスワードポリシーを確認
    - 現在の設定: 最小 8 文字、大文字・小文字・数字・特殊文字すべて必須
    - 現在の実装: 最小 8 文字、小文字 1 つ以上、大文字 1 つ以上、数字 1 つ以上、特殊文字 1 つ以上
    - _要件: 1.2, 4.2_

  - [ ] 3.10.2 ローカル開発環境（Magneto）のパスワードポリシー設定

    - Magneto の設定ファイルでパスワードポリシーを確認
    - デフォルトで Cognito 互換のポリシーが適用されている
    - ローカル環境でサインアップテスト（例：Test1234）を実施
    - _要件: 1.2, 4.2_

  - [ ] 3.10.3 本番環境 Cognito のパスワードポリシー設定

    - Terraform で Cognito User Pool のパスワードポリシーを定義
    - `password_policy` ブロックで以下を設定:
      - `minimum_length = 8`
  ...
  - [ ] 3.10.4 フロントエンドのバリデーション更新

    - `SignupModal.tsx` のパスワードバリデーションを確認
    - 8 文字以上、英字 1 つ以上、数字 1 つ以上の要件を実装済み
    - エラーメッセージが適切に表示されることを確認
    - _要件: 1.2, 4.2_

  - [ ] 3.10.5 パスワードポリシーのドキュメント更新

    - README.md にパスワード要件を記載
    - ユーザー向けドキュメントにパスワードポリシーを追加
    - 開発者向けドキュメントに Cognito 設定手順を追加
    - _要件: 1.2_

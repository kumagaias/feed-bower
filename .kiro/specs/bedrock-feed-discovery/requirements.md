...
**ユーザーストーリー:** ユーザーとして、キーワードに基づいて推奨されたフィードを自動的にBowerに登録してほしい。手動で1つずつフィードを追加する手間を省くため。

#### 受け入れ基準

1. Feed Serviceはキーワードとbower_idを受け取り、推奨フィードを自動的にBowerに追加するエンドポイントを提供すること
2. Feed Serviceは推奨フィードを取得する前に、ユーザーがBowerの所有者であることを検証すること
3. Feed Serviceは既にBowerに存在するフィードURLをスキップすること
4. Feed Serviceは各フィードURLが有効なRSS/Atomフィードであることを検証してから追加すること
5. Feed Serviceは追加されたフィード数、スキップされたフィード数、失敗したフィード数を含むサマリーを返すこと

### 要件10

**ユーザーストーリー:** ユーザーとして、Bowerを作成する際にキーワードから自動的にフィードを登録してほしい。新しいBowerをすぐに使い始められるようにするため。

#### 受け入れ基準

1. Bower Serviceは新しいBowerを作成する際、キーワードに基づいて自動的にフィードを追加するオプションを提供すること
2. Bower Serviceはauto_register_feedsフラグがtrueの場合、Bower作成後に自動的にフィード推奨と登録を実行すること
3. Bower Serviceは自動登録するフィードの最大数を制限すること（デフォルト5件）
4. Bower Serviceはフィード自動登録が失敗してもBower作成を失敗させないこと
5. Bower Serviceは自動登録されたフィード数をレスポンスに含めること

...
# ゲストユーザー作成スクリプト
# Usage: ./scripts/create-guest-user.sh [environment]

set -e

ENVIRONMENT=${1:-production}
EMAIL="feed-bower-guest@example.com"
PASSWORD="aj8JipqmRCYG"
USER_POOL_ID=""
...
if aws cognito-idp admin-get-user --user-pool-id "$USER_POOL_ID" --username "$EMAIL" 2>/dev/null; then
    echo "⚠️  Guest user already exists"
    echo ""
    read -p "Do you want to delete and recreate? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "🗑️  Deleting existing guest user..."
